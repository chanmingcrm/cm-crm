...
import com.platform.mesh.core.constants.*;
...
public class ExcelUtil {
    ...
    public static void exportTemp(List<HeadDTO> headDTOS, String fileName, HttpServletResponse response) {
        //获取模拟样例数据
        List<List<DataBO>> dataList = getTempDataBOList(headDTOS);
        //导出数据
        exportData(headDTOS,dataList,fileName,response);
    }
     ...
     * @author 蝉鸣
     */
    public static List<List<DataBO>> getTempDataBOList(List<HeadDTO> headDTOS) {
        if(CollUtil.isEmpty(headDTOS)){
            return CollUtil.newArrayList();
        }
        //根据表头顺序组装数据
        List<DataBO> list = headDTOS.stream().map(headDTO -> {
            DataBO dataBO = new DataBO();
            dataBO.setKey(headDTO.getHeadName());
            //单元格最大长度32767
            Object value;
            if(CollUtil.isNotEmpty(headDTO.getHeadValid())){
                value = CollUtil.getFirst(headDTO.getHeadValid());
            }else if(headDTO.getHeadMac().endsWith(StrConst.TIME_SUFFIX)){
                value = LocalDateTime.now().format(DateTimeFormatter.ofPattern(DateConst.YYYY_MM_DD_HH_MM_SS));
            }else{
                value = ExcelConst.DATA_EXAMPLE;
            }
            dataBO.setValue(value);
            return dataBO;
        }).toList();
        List<List<DataBO>> dataBOS = CollUtil.newArrayList();
        dataBOS.add(list);
        return dataBOS;
    }
     ...
     * @author 蝉鸣
     */
    public static void exportData(List<HeadDTO> headDTOS, List<List<DataBO>> dataList, String fileName, HttpServletResponse response) {
        //获取表头
        ...
        try{
            ...
            excelWriter = FesodSheet.write(response.getOutputStream()).head(headList).inMemory(Boolean.TRUE).build();
            WriteSheet sheet = FesodSheet.writerSheet()
                    .registerWriteHandler(new TempExportRowWriteHandler())
                    .registerWriteHandler(new TempExportCellWriteHandler(headDTOS))
                    .build();
            excelWriter.write(toExcelRows(dataList), sheet);
        }catch (Exception e ){
    ...
    public static void exportData(Function<Integer,PageVO<Object>> function, List<HeadDTO> headDTOS, String fileName, HttpServletResponse response) {
        ...
        try{
            ...
            fileName = URLEncoder.encode(fileName, StandardCharsets.UTF_8).replaceAll(SymbolConst.PATTERN_PLUS, SymbolConst.PATTERN_PLUS_ESCAPE);
            response.setHeader(HttpHeaders.CONTENT_DISPOSITION, HttpConst.FILE_NAME_PREFIX_SHEET + fileName + MimeTypeConst.SUFFIX_EXCEL_X);
            excelWriter = FesodSheet.write(response.getOutputStream()).head(headList)
                    .registerWriteHandler(new DataExportCellWriteHandler()).build();
            ...
            while (true) {
                ...
                List<List<DataBO>> dataList = getDataBOList(headDTOS, pageVO.getRecords());
                excelWriter.write(toExcelRows(dataList), sheet);
                pageNum ++;
            }
        }catch (Exception e ){
    ...
    public static List<List<DataBO>> getDataBOList(List<HeadDTO> headDTOS, List<Object> pageDataList) {
        ...
        return pageDataList.stream().map(record -> {
            ...
            headDTOS.forEach(headDTO -> {
                ...
                if(StrUtil.toString(value).length() >= ExcelConst.CELL_MAX_LENGTH){
     ...
     * 功能描述:
     * 〈新版本转换器无法正常识别,转换程字符串〉
     * @return 正常返回:{@link List<List<String>>}
     * @author 蝉鸣
     */
    private static List<List<String>> toExcelRows(List<List<DataBO>> dataList) {
        if(CollUtil.isEmpty(dataList)){
            return CollUtil.newArrayList();
        }
        return dataList.stream()
                .map(row -> row.stream()
                        .map(dataBO -> StrUtil.toString(dataBO.getValue()))
    ...
    public static Object parseValue(HeadDTO headDTO, Object value){
        ...
        if(ObjectUtil.isEmpty(enumByDesc)){
            ...
            if(DataTypeEnum.JSON_OBJECT.getValue().equals(enumByDesc.getDataType())){
                if(JSONUtil.isTypeJSON(JSONUtil.toJsonStr(value))){
