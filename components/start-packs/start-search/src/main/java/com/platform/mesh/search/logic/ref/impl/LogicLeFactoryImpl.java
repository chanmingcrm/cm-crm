...
public class LogicLeFactoryImpl implements LogicRefService {
    ...
    public <T> QueryWrapper<T> sqlWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        return wrapper.le(condDTO.getColumnMac(), condDTO.getSearchValues());
    }
    ...
    public <T> QueryWrapper<T> sqlDataWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        wrapper.le(SearchColumnConst.DATA_COLUMN_VALUE, condDTO.getSearchValues());
        return wrapper;
    }
    ...
    public Query esQuery(CondDTO condDTO){
        ...
        return QueryBuilders.range(le->{
//            le.date(builder->builder.field(condDTO.getColumnMac()).gte(CollUtil.getFirst(condDTO.getSearchValues())));
//            le.term(builder->builder.field(condDTO.getColumnMac()).gte(CollUtil.getFirst(condDTO.getSearchValues())));
//            le.number(builder->builder.field(condDTO.getColumnMac()).gte(NumberUtil.parseDouble(CollUtil.getFirst(condDTO.getSearchValues()))));
//            le.untyped(builder->builder.field(condDTO.getColumnMac()).gte(JsonData.of(CollUtil.getFirst(condDTO.getSearchValues()))));
            return le;
        });
    }
}
