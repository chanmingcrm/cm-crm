 ...
 */
@Service
public class LogicLtFactoryImpl implements LogicRefService {

    private final static Logger log = LoggerFactory.getLogger(LogicLtFactoryImpl.class);
    ...
    public <T> QueryWrapper<T> sqlWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        return wrapper.lt(condDTO.getColumnMac(), condDTO.getSearchValues());
    }
    ...
    public <T> QueryWrapper<T> sqlDataWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        wrapper.lt(SearchColumnConst.DATA_COLUMN_VALUE, condDTO.getSearchValues());
        return wrapper;
    }
