...
public class LogicEqFactoryImpl implements LogicRefService {
    ...
    public <T> QueryWrapper<T> sqlWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        return wrapper.eq(condDTO.getColumnMac(), CollUtil.getFirst(condDTO.getSearchValues()));
    };
    ...
    public <T> QueryWrapper<T> sqlDataWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        wrapper.eq(SearchColumnConst.DATA_COLUMN_VALUE, condDTO.getSearchValues());
        return wrapper;
    };
