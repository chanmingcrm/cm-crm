 ...
 */
@Service
public class LogicInFactoryImpl implements LogicRefService {

    private final static Logger log = LoggerFactory.getLogger(LogicInFactoryImpl.class);
    ...
    public <T> QueryWrapper<T> sqlWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        return wrapper.in(condDTO.getColumnMac(), condDTO.getSearchValues());
    }
    ...
    public <T> QueryWrapper<T> sqlDataWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        wrapper.in(SearchColumnConst.DATA_COLUMN_VALUE, condDTO.getSearchValues());
        return wrapper;
    }
    ...
    public Query esQuery(CondDTO condDTO){
        ...
        return QueryBuilders.terms(in->{
            in.field(condDTO.getColumnMac());
            in.terms(item -> item.value(condDTO.getSearchValues().stream().map(FieldValue::of).toList()));
            return in;
        });
    }
}
