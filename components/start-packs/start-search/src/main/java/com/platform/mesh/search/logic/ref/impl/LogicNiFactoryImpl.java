 ...
 */
@Service
public class LogicNiFactoryImpl implements LogicRefService {

    private final static Logger log = LoggerFactory.getLogger(LogicNiFactoryImpl.class);
    ...
    public <T> QueryWrapper<T> sqlWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        return wrapper.notIn(condDTO.getColumnMac(), condDTO.getSearchValues());
    }
    ...
    public <T> QueryWrapper<T> sqlDataWrapper(QueryWrapper<T> wrapper, CondDTO condDTO){
        ...
        wrapper.notIn(SearchColumnConst.DATA_COLUMN_VALUE, condDTO.getSearchValues());
        return wrapper;
    }
