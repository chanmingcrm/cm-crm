...
public class LogicNotFactoryImpl implements LogicTypeService {
        ...
        wrapper.and(andWrapper->{
            for (CondDTO condDTO : condDTOS) {
                andWrapper.not(notWrapper->{
                    LogicRefEnum condition = condDTO.getCondRef();
                    LogicRefService logicRefService = logicRefFactory.getLogicRefService(condition);
                    logicRefService.sqlDataWrapper(notWrapper,condDTO);
                });
            }
        });
        return wrapper;
    ...
    public Map<EsBoolEnum, List<Query>> esBoolQuery(List<CondDTO> condDTOS){
        ...
        boolMap.put(EsBoolEnum.MUST_NOT,queryList);
        return boolMap;
    }
